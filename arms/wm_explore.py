""" WM-EXPLORE arm: the combined method + a bandit-gated novelty
    bonus at act-time selection during ONLINE COLLECTION only.

    Training is the combined arm's, byte for byte: QC-FQL losses, the critic's
    ...
    Sep 3 and never run): exploit episodes are exactly the combined arm, explore
    ...
    (bandit arm 0) scores candidates identically to the combined arm -- gate 0
    zeroes the bonus, leaving the critic's argmax. wm_explore.beta=0 disables
...
class WMExploreArm(CombinedArm):
    name = 'wm_explore'

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        e = self.config.wm_explore
        assert e.num_dyn >= 2 or e.beta == 0.0, \
            'novelty needs a dynamics ensemble: set wm_explore.num_dyn >= 2'
    ...
    def build_model(self):
        ...
        self.model = TDMPC2Model(self.obs_dim, self.action_dim, self.device,
                                 ...
                                 num_dyn=self.config.wm_explore.num_dyn)
        return self.model
    ...
    def describe(self):
        e = self.config.wm_explore
        return (f'{self.name}: combined best-of-{self.chunk.select_n} '
