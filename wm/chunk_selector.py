...
class ChunkSelector:
    """ Best-of-N chunk selection at act time.
        ...
        The wm_explore arm adds a bounded novelty bonus on top of the critic's
        ...
        stateless apart from the wm_explore arm's learning-progress gate. """
    ...
    def __init__(self, model, policy, action_dim, chunk_len, n, gamma, device,
        ...
        """ model: a TDMPC2Model (wm_explore arm), or None (combined arm).
            rollout_chunks (wm_explore arm): imagine this many chunks ahead when
            ...
            bonus_beta (wm_explore arm): weight of the novelty bonus. 0 disables
            it and gives exactly the combined arm. Training-time only:
    ...
    def report_episode_return(self, ep_return):
        """ Learning-progress gate. Call once per finished REAL online
            ...
            Improving returns give frac -> 1 -> g -> 0 (become the combined arm).
    ...
    def begin_episode(self):
        """ Called by the loop at the start of every REAL online episode
            ...
            then explore). Arm 0 = exploit, g = 0, exactly the combined arm for
    ...
    def _select_uncertainty_scaled(self, feat_n, cands, qs, critic_score):
        """ EXPLORE arm. The critic orders the candidates; novelty may only
            ...
            g        learning-progress gate in [0, 1], see
                     ...
                     while they improve (the arm becomes the combined arm)
            ...
            agree, s_i -> 0 and the arm becomes the combined arm -- continuously,
