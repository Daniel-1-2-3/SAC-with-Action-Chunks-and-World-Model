"""COMBINED arm in JAX: ColinQiyangLi/qc@48283b4 agents/acfql.py plus the
best-of-N candidate selection of the combined method. Upstream code is
verbatim except where a `COMBINED PATCH` comment says otherwise:

  - get_config gains `actor_num_candidates` (chunk.select_n) and
    `candidate_source` ('actor' | 'bc').
  - sample_actions in distill-ddpg mode with actor_num_candidates > 1
    draws that many candidate chunks (from the one-step actor, or from
    the flow policy with candidate_source='bc'), scores each with the
    ONLINE critic (q_agg over the ensemble) and returns the argmax.
    Used at act time and, through evaluation.py, at eval time.
  - critic_loss bootstraps on ONE plain one-step-actor sample at s'
    (sample_onestep_actions), never on the act-time best-of-N: training
    stays exactly QC-FQL's, only acting changes -- the combined arm.

With actor_num_candidates=1 every patched path reduces to upstream
QC-FQL (actor_type='distill-ddpg') unchanged.
"""
...
class ACFQLAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        # COMBINED PATCH: in distill-ddpg mode the TD target always uses ONE
        # plain one-step-actor sample at s', never the act-time best-of-N.
        # Identical to upstream for actor_num_candidates=1 (sample_actions
        # was exactly this); for > 1 it keeps training exactly QC-FQL's.
        if self.config["actor_type"] == "distill-ddpg":
            next_actions = self.sample_onestep_actions(batch['next_observations'][..., -1, :], rng=sample_rng)
        else:
    ...
    def sample_actions(
        ...
        if self.config["actor_type"] == "distill-ddpg":
            if self.config["actor_num_candidates"] > 1:
                # COMBINED PATCH: best-of-N over actor_num_candidates
                # candidate chunks -- from the one-step actor (default) or
                # the flow policy (candidate_source='bc') -- scored by the
                # ONLINE critic with q_agg, argmax executed. Same gather as
                # the best-of-n branch below; only the sampler differs.
                ...
                bsize = len(indices)
                actions = jnp.reshape(actions, (-1, n, action_dim))[jnp.arange(bsize), indices, :].reshape(
                    bshape + (action_dim,))
            else:
                actions = self.sample_onestep_actions(observations, rng=rng)
    ...
    def sample_onestep_actions(
        ...
        # COMBINED PATCH: upstream's distill-ddpg sample_actions body,
        # verbatim, as its own method -- one noise vector through the
        # one-step actor, clipped. The TD target uses this; so does
        # sample_actions when actor_num_candidates == 1.
        noises = jax.random.normal(
            rng,
            (
                *observations.shape[: -len(self.config['ob_dims'])],  # batch_size
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            ...
            # COMBINED PATCH (the two knobs of the combined arm):
            actor_num_candidates=1,  # act/eval-time best-of-N over candidates in distill-ddpg mode (chunk.select_n); 1 = plain QC-FQL
            candidate_source='actor',  # 'actor' = one-step actor, 'bc' = flow policy (chunk.candidate_source)
