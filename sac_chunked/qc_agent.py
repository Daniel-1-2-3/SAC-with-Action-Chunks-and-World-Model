...
class QCAgent(ChunkAgent):
    ...
    def __init__(self, repr_dim, action_dim, chunk_len, device, lr, hidden_dim,
                 ...
                 critic_layer_norm=True, compile_nets=False, target_mode='bestofn'):
        # ChunkAgent.__init__ is deliberately NOT called: QC has no
        # actor_onestep_flow and no alpha.
        assert target_mode in ('bestofn', 'single'), target_mode
        self.target_mode = target_mode   # bootstrap: best-of-N (paper) or one BC sample
        self.device = device
    ...
    def chunk_target_values(self, next_feats):
        ...
        if self.target_mode == 'single':
            # ATTRIBUTION variant: one BC sample at s', no argmax (removes the
            # max-over-N bias from the bootstrap; acting is unchanged).
            a_star = self.sample_chunks_bc(next_feats, 1)
        else:
